...
def calculate_average_stats(row, level_string):
    ...
    Applies stat caps: 20 unpromoted (30 for Luck, 60 for HP), 30 promoted (60 for HP).
    ...
    try:
        ...
        if len(level_parts) == 1:
            ...
            # Apply promoted stat cap (30 for all stats except HP at 60)
            for stat in avg_stats:
                if stat == 'HP':
                    avg_stats[stat] = min(avg_stats[stat], 60)
                else:
...
async def unit(ctx, name: str, levels: str = None):
    ...
    Usage:
        ...
        /unit [name] [level] - Show average stats at level (e.g., /unit Rena 10)
        /unit [name] [level]/[level] - Show average stats after promotion (e.g., /unit Rena 10/5)
    """
