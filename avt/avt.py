...
def calculate_average_stats(row, level_string):
    ...
    All stats cap at 20 regardless of tier (except Luck at 30).
    ...
    try:
        ...
        if len(level_parts) == 1:
            ...
            # Apply stat caps (20 for all stats, 30 for Luck)
            ...
            # Apply stat caps before promotion
            ...
            # Apply stat caps (20 for all stats, 30 for Luck - same as unpromoted)
            for stat in avg_stats:
                if stat == 'HP':
                    avg_stats[stat] = min(avg_stats[stat], 60)
                else:
...
async def unit(ctx, name: str, levels: str = None):
    ...
    Usage:
        ...
        /unit [name] [level] - Show average stats at level (e.g., /unit Ezekiel 10)
        /unit [name] [level]/[level] - Show average stats after promotion (e.g., /unit Ezekiel 10/5)
    """
